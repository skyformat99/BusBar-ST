/**
 * @file lpc175x_eth.c
 * @brief LPC1750 Ethernet MAC controller
 *
 * @section License
 *
...
void lpc175xEthInitGpio(NetInterface *interface)
{
   ...
   //Configure P1.16 (ENET_MDC) and P1.17 (ENET_MDIO) as GPIOs
   LPC_PINCON->PINSEL3 &= ~(PINSEL3_P1_16_MASK | PINSEL3_P1_17_MASK);
   LPC_PINCON->PINSEL3 |= PINSEL3_P1_16_GPIO | PINSEL3_P1_17_GPIO;

   //SMI software implementation to drive MDC and MDIO
   LPC_GPIO1->FIOCLR = LPC175X_MDC;
   LPC_GPIO1->FIODIR |= LPC175X_MDC;
   LPC_GPIO1->FIODIR &= ~LPC175X_MDIO;
}
...
void lpc175xEthWriteSmi(uint32_t data, uint_t length)
{
   ...
   while(length--)
   {
      //Write MDIO
      if(data & 0x80000000)
         LPC_GPIO1->FIOSET = LPC175X_MDIO;
      else
         LPC_GPIO1->FIOCLR = LPC175X_MDIO;

      //Assert MDC
      usleep(1);
      LPC_GPIO1->FIOSET = LPC175X_MDC;
      //Deassert MDC
      usleep(1);
      LPC_GPIO1->FIOCLR = LPC175X_MDC;
...
uint32_t lpc175xEthReadSmi(uint_t length)
{
   ...
   while(length--)
   {
      ...
      //Check MDIO state
      if(LPC_GPIO1->FIOPIN & LPC175X_MDIO)
         data |= 0x00000001;
   }
