...
#pragma data_alignment = 16
static uint16_t rxBufferDesc[MK60_ETH_RX_BUFFER_COUNT][16];
...
error_t mk60EthInit(NetInterface *interface)
{
   ...
   NVIC_SetPriorityGrouping(MK60_ETH_IRQ_PRIORITY_GROUPING);

   //Configure ENET transmit interrupt priority
   NVIC_SetPriority(ENET_Transmit_IRQn, NVIC_EncodePriority(MK60_ETH_IRQ_PRIORITY_GROUPING,
      MK60_ETH_IRQ_GROUP_PRIORITY, MK60_ETH_IRQ_SUB_PRIORITY));

   //Configure ENET receive interrupt priority
   NVIC_SetPriority(ENET_Receive_IRQn, NVIC_EncodePriority(MK60_ETH_IRQ_PRIORITY_GROUPING,
      MK60_ETH_IRQ_GROUP_PRIORITY, MK60_ETH_IRQ_SUB_PRIORITY));

   //Configure ENET error interrupt priority
   NVIC_SetPriority(ENET_Error_IRQn, NVIC_EncodePriority(MK60_ETH_IRQ_PRIORITY_GROUPING,
      MK60_ETH_IRQ_GROUP_PRIORITY, MK60_ETH_IRQ_SUB_PRIORITY));
...
void mk60EthInitBufferDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < MK60_ETH_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < MK60_ETH_RX_BUFFER_COUNT; i++)
   {
   ...
   ENET->MRBR = MK60_ETH_RX_BUFFER_SIZE;
}
...
void mk60EthEventHandler(NetInterface *interface)
{
   ...
   if(status & ENET_EIR_RXF_MASK)
   {
      ...
      do
      {
         ...
         error = mk60EthReceivePacket(interface,
   ...
   const NetBuffer *buffer, size_t offset)
{
   ...
   if(length > MK60_ETH_TX_BUFFER_SIZE)
   {
   ...
   //Check current index
   if(txBufferIndex < (MK60_ETH_TX_BUFFER_COUNT - 1))
   {
   ...
   uint8_t *buffer, size_t size, size_t *length)
{
   ...
   if(!(rxBufferDesc[rxBufferIndex][0] & HTOBE16(ENET_RBD0_E)))
   {
      ...
      //Check current index
      if(rxBufferIndex < (MK60_ETH_RX_BUFFER_COUNT - 1))
      {
