...
void same70EthEventHandler(NetInterface *interface)
{
   ...
   if(rsr & (GMAC_RSR_HNO | GMAC_RSR_RXOVR | GMAC_RSR_REC | GMAC_RSR_BNA))
   {
      ...
      while(1)
      {
         ...
         length = same70EthReceivePacket(interface, interface->ethFrame, ETH_MAX_FRAME_SIZE);
...
uint_t same70EthReceivePacket(NetInterface *interface,
   uint8_t *buffer, size_t size)
{
   ...
   for(i = 0; i < SAME70_ETH_RX_BUFFER_COUNT; i++)
   {
      ...
      if(index >= SAME70_ETH_RX_BUFFER_COUNT)
         index -= SAME70_ETH_RX_BUFFER_COUNT;
