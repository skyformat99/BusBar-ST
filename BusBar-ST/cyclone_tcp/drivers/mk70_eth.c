...
static uint8_t txBuffer[MK70_ETH_TX_BUFFER_COUNT][MK70_ETH_TX_BUFFER_SIZE];
//RX buffer
#pragma data_alignment = 16
static uint8_t rxBuffer[MK70_ETH_RX_BUFFER_COUNT][MK70_ETH_RX_BUFFER_SIZE];
//TX buffer descriptors
#pragma data_alignment = 16
static uint16_t txBufferDesc[MK70_ETH_TX_BUFFER_COUNT][16];
//RX buffer descriptors
#pragma data_alignment = 16
static uint16_t rxBufferDesc[MK70_ETH_RX_BUFFER_COUNT][16];
...
/**
 * @brief Kinetis K70 Ethernet MAC driver
 **/

const NicDriver mk70EthDriver =
{
   NIC_TYPE_ETHERNET,
   ETH_MTU,
   mk70EthInit,
   ...
   mk70EthEventHandler,
   mk70EthSendPacket,
   mk70EthSetMulticastFilter,
   mk70EthUpdateMacConfig,
   mk70EthWritePhyReg,
   mk70EthReadPhyReg,
   TRUE,
   TRUE,
...
/**
 * @brief Kinetis K70 Ethernet MAC initialization
 * @param[in] interface Underlying network interface
 * @return Error code
 **/

error_t mk70EthInit(NetInterface *interface)
{
   error_t error;
   uint32_t value;

   //Debug message
   TRACE_INFO("Initializing Kinetis K70 Ethernet MAC...\r\n");
   ...
   mk70EthInitBufferDesc(interface);
   ...
   NVIC_SetPriorityGrouping(MK70_ETH_IRQ_PRIORITY_GROUPING);

   //Configure ENET transmit interrupt priority
   NVIC_SetPriority(ENET_Transmit_IRQn, NVIC_EncodePriority(MK70_ETH_IRQ_PRIORITY_GROUPING,
      MK70_ETH_IRQ_GROUP_PRIORITY, MK70_ETH_IRQ_SUB_PRIORITY));

   //Configure ENET receive interrupt priority
   NVIC_SetPriority(ENET_Receive_IRQn, NVIC_EncodePriority(MK70_ETH_IRQ_PRIORITY_GROUPING,
      MK70_ETH_IRQ_GROUP_PRIORITY, MK70_ETH_IRQ_SUB_PRIORITY));

   //Configure ENET error interrupt priority
   NVIC_SetPriority(ENET_Error_IRQn, NVIC_EncodePriority(MK70_ETH_IRQ_PRIORITY_GROUPING,
      MK70_ETH_IRQ_GROUP_PRIORITY, MK70_ETH_IRQ_SUB_PRIORITY));
...
void mk70EthInitGpio(NetInterface *interface)
{
...
void mk70EthInitBufferDesc(NetInterface *interface)
{
   uint_t i;
   uint32_t address;
   ...
   for(i = 0; i < MK70_ETH_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < MK70_ETH_RX_BUFFER_COUNT; i++)
   {
   ...
   ENET->MRBR = MK70_ETH_RX_BUFFER_SIZE;
}


/**
 * @brief Kinetis K70 Ethernet MAC timer handler
 *
...
void mk70EthTick(NetInterface *interface)
{
...
void mk70EthEnableIrq(NetInterface *interface)
{
...
void mk70EthDisableIrq(NetInterface *interface)
{
...
/**
 * @brief Kinetis K70 Ethernet MAC event handler
 * @param[in] interface Underlying network interface
 **/

void mk70EthEventHandler(NetInterface *interface)
{
   error_t error;
   size_t length;
   uint32_t status;
   ...
   if(status & ENET_EIR_RXF_MASK)
   {
      ...
      do
      {
         ...
         error = mk70EthReceivePacket(interface,
...
error_t mk70EthSendPacket(NetInterface *interface,
   const NetBuffer *buffer, size_t offset)
{
   ...
   if(length > MK70_ETH_TX_BUFFER_SIZE)
   {
   ...
   //Check current index
   if(txBufferIndex < (MK70_ETH_TX_BUFFER_COUNT - 1))
   {
...
error_t mk70EthReceivePacket(NetInterface *interface,
   uint8_t *buffer, size_t size, size_t *length)
{
   ...
   if(!(rxBufferDesc[rxBufferIndex][0] & HTOBE16(ENET_RBD0_E)))
   {
      ...
      //Check current index
      if(rxBufferIndex < (MK70_ETH_RX_BUFFER_COUNT - 1))
      {
...
error_t mk70EthSetMulticastFilter(NetInterface *interface)
{
   uint_t i;
   uint_t k;
   uint32_t crc;
   ...
   //Debug message
   TRACE_DEBUG("Updating Kinetis K70 hash table...\r\n");
   ...
   for(i = 0; i < MAC_MULTICAST_FILTER_SIZE; i++)
   {
      ...
      if(entry->refCount > 0)
      {
         ...
         crc = mk70EthCalcCrc(&entry->addr, sizeof(MacAddr));
...
error_t mk70EthUpdateMacConfig(NetInterface *interface)
{
...
void mk70EthWritePhyReg(uint8_t phyAddr, uint8_t regAddr, uint16_t data)
{
   uint32_t value;
...
uint16_t mk70EthReadPhyReg(uint8_t phyAddr, uint8_t regAddr)
{
   uint32_t value;
...
uint32_t mk70EthCalcCrc(const void *data, size_t length)
{
   uint_t i;
   uint_t j;
