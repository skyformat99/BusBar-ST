...
void am335xEthEventHandler(NetInterface *interface)
{
   ...
   do
   {
      ...
      if(!(rxCurBufferDesc->word3 & CPSW_RX_WORD3_OWNER))
      {
            ...
            (rxCurBufferDesc->word3 & CPSW_RX_WORD3_EOP))
         {
            ...
            if(!(rxCurBufferDesc->word3 & CPSW_RX_WORD3_PKT_ERROR))
            {
               ...
               CacheDataInvalidateBuff(rxCurBufferDesc->word1, n);

               //Sanity check
               if(interface != NULL)
               {
                  //Copy data from the receive buffer
                  memcpy(interface->ethFrame, (uint8_t *) rxCurBufferDesc->word1, n);
      ...
      if(!error)
      {
         ...
         nicProcessPacket(interface, interface->ethFrame, n);
      }
   ...
   const NetBuffer *buffer, size_t offset)
{
   ...
   //Make sure that the data is written to memory. Clean the portion of cache
   //to make it coherent with the memory
   CacheDataCleanBuff(txCurBufferDesc1->word1, length);
   ...
   const NetBuffer *buffer, size_t offset)
{
   ...
   //Make sure that the data is written to memory. Clean the portion of cache
   //to make it coherent with the memory
   CacheDataCleanBuff(txCurBufferDesc2->word1, length);
