...
/**
 * @brief Kinetis K64 Ethernet MAC driver
 **/

const NicDriver mk64EthDriver =
{
   NIC_TYPE_ETHERNET,
   ETH_MTU,
   mk64EthInit,
   ...
   mk64EthEventHandler,
   mk64EthSendPacket,
   mk64EthSetMulticastFilter,
   mk64EthUpdateMacConfig,
   mk64EthWritePhyReg,
   mk64EthReadPhyReg,
   TRUE,
   TRUE,
...
/**
 * @brief Kinetis K64 Ethernet MAC initialization
...
error_t mk64EthInit(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing Kinetis K64 Ethernet MAC...\r\n");
   ...
   mk64EthInitBufferDesc(interface);
   ...
   NVIC_SetPriorityGrouping(MK64_ETH_IRQ_PRIORITY_GROUPING);

   //Configure ENET transmit interrupt priority
   NVIC_SetPriority(ENET_Transmit_IRQn, NVIC_EncodePriority(MK64_ETH_IRQ_PRIORITY_GROUPING,
      MK64_ETH_IRQ_GROUP_PRIORITY, MK64_ETH_IRQ_SUB_PRIORITY));

   //Configure ENET receive interrupt priority
   NVIC_SetPriority(ENET_Receive_IRQn, NVIC_EncodePriority(MK64_ETH_IRQ_PRIORITY_GROUPING,
      MK64_ETH_IRQ_GROUP_PRIORITY, MK64_ETH_IRQ_SUB_PRIORITY));

   //Configure ENET error interrupt priority
   NVIC_SetPriority(ENET_Error_IRQn, NVIC_EncodePriority(MK64_ETH_IRQ_PRIORITY_GROUPING,
      MK64_ETH_IRQ_GROUP_PRIORITY, MK64_ETH_IRQ_SUB_PRIORITY));
...
/**
 * @brief Kinetis K64 Ethernet MAC timer handler
 *
...
/**
 * @brief Kinetis K64 Ethernet MAC event handler
 * @param[in] interface Underlying network interface
 **/
...
void mk64EthEventHandler(NetInterface *interface)
{
   ...
   if(status & ENET_EIR_RXF_MASK)
   {
      ...
      do
      {
         ...
         error = mk64EthReceivePacket(interface,
   ...
   const NetBuffer *buffer, size_t offset)
{
   ...
   if(length > MK64_ETH_TX_BUFFER_SIZE)
   {
   ...
   //Check current index
   if(txBufferIndex < (MK64_ETH_TX_BUFFER_COUNT - 1))
   {
   ...
   uint8_t *buffer, size_t size, size_t *length)
{
   ...
   if(!(rxBufferDesc[rxBufferIndex][0] & HTOBE16(ENET_RBD0_E)))
   {
      ...
      //Check current index
      if(rxBufferIndex < (MK64_ETH_RX_BUFFER_COUNT - 1))
      {
...
error_t mk64EthSetMulticastFilter(NetInterface *interface)
{
   uint_t i;
   uint_t k;
   uint32_t crc;
   ...
   //Debug message
   TRACE_DEBUG("Updating Kinetis K64 hash table...\r\n");
   ...
   for(i = 0; i < MAC_MULTICAST_FILTER_SIZE; i++)
   {
      ...
      if(entry->refCount > 0)
      {
         ...
         crc = mk64EthCalcCrc(&entry->addr, sizeof(MacAddr));
...
error_t mk64EthUpdateMacConfig(NetInterface *interface)
{
