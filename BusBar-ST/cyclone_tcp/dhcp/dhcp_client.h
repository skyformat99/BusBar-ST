...
typedef struct
{
   NetInterface *interface;                           ///<Network interface to configure  网络接口配置
   char_t hostname[DHCP_CLIENT_MAX_HOSTNAME_LEN + 1]; //Host name   主机名
   bool_t rapidCommit;                                ///<Quick configuration using rapid commit 快速配置使用
   bool_t manualDnsConfig;                            ///<Force manual DNS configuration
   systime_t timeout;                                 ///<DHCP configuration timeout  DHCP timeout配置
   DhcpTimeoutCallback timeoutEvent;                  ///<DHCP configuration timeout event 配置 DHCP 超时事件
