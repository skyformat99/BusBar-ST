...
error_t netInit(void)
{
   ...
   //Memory pool initialization   内存池初始化
   error = memPoolInit();
   //Any error to report?
   if(error) 
		 return error;
   ...
   //Loop through network interfaces  遍历网络接口
   ...
   //Create a mutex to prevent simultaneous access to the callback table 创建一个互斥锁来同时防止回调表
   if(!osCreateMutex(&callbackTableMutex))
   {
      //Failed to create mutex  未能创建互斥现象
      return ERROR_OUT_OF_RESOURCES;
   }
