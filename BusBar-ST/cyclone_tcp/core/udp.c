...
/**
 * @brief Incoming UDP datagram processing 处理传入的UDP数据报
   ...
   IpPseudoHeader *pseudoHeader, const NetBuffer *buffer, size_t offset)
{
   ...
   //Retrieve the length of the UDP datagram  接收到的UDP数据报
   ...
   //Point to the UDP header 指向UDP头
   ...
   for(i = 0; i < SOCKET_MAX_COUNT; i++)
   {
      ...
      if(socket->interface && socket->interface != interface)
         continue;
      //Check destination port number 检查目的端口号
      ...
      //An IPv4 packet was received? 接收到IPv4的数据包
   ...
   {
      //Invoke user callback, if any 调用回调函数
...
/**
 * @brief Send a UDP datagram 发送一个数据包
...
/**
 * @brief Send a UDP datagram (raw interface) 发送一个UDP数据报
 * @param[in] interface Underlying network interface
...
/**
 * @brief Receive data from a UDP socket 从UDP套接字接收数据
 * @param[in] socket Handle referencing the socket
...
/**
 * @brief Allocate a buffer to hold a UDP packet   UDP申请动态内存
 * @param[in] length Desired payload length  所需的长度
...
/**
 * @brief Update UDP related events 更新UDP相关事件
 * @param[in] socket Handle referencing the socket
 **/
...
/**
 * @brief Register user callback  用户注册回调
 * @param[in] interface Underlying network interface 网卡
 * @param[in] port UDP port number  UDP网卡端口
 * @param[in] callback Callback function to be called when a datagram is received 接收回调函数
 * @param[in] params Callback function parameter (optional) 传给回调函数的参数
 * @return Error code
 **/
...
/**
 * @brief Unregister user callback 注销用户回调函数
 * @param[in] interface Underlying network interface 网卡
 * @param[in] port UDP port number  端口号
 * @return Error code
 **/
...
/**
 * @brief Invoke user callback  用户调用回调函数
 * @param[in] interface Underlying network interface 网卡
 * @param[in] pseudoHeader UDP pseudo header  UDP伪头
 * @param[in] header UDP header   UDP头
 * @param[in] buffer Multi-part buffer containing the payload  UDP数据区
