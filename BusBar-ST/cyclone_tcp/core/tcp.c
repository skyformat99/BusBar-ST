...
/**
 * @brief Get an ephemeral port number 得到一个短暂的端口号
...
/**
 * @brief Establish a TCP connection 建立TCP连接
...
/**
 * @brief Place a socket in the listening state监听状态
 *
...
/**
 * @brief Permit an incoming connection attempt on a TCP socket 接收TCP请求
 * @param[in] socket Handle to a socket previously placed in a listening state
 * @param[out] clientIpAddr IP address of the client  客户端的IP地址
 * @param[out] clientPort Port number used by the client 客户机使用的端口号
...
/**
 * @brief Send data to a connected socket 发送数据
 * @param[in] socket Handle that identifies a connected socket 套接字
 * @param[in] data Pointer to a buffer containing the data to be transmitted 数据缓冲区
 * @param[in] length Number of bytes to be transmitted 传输的长度
 * @param[out] written Actual number of bytes written (optional parameter)  真实传输的多少
 * @param[in] flags Set of flags that influences the behavior of this function 标志
 * @return Error code
 **/
...
/**
 * @brief Receive data from a connected socket  TCP接收函数
...
/**
 * @brief Abort an existing TCP connection 中止TCP连接
