...
TlsContext *tlsInit(void)
{
   ...
   context->entity = TLS_CONNECTION_END_CLIENT;    //选择为客户端模式
   //Default TLS version
...
#endif
#if (TLS_ECDHE_RSA_SUPPORT == ENABLED || TLS_ECDHE_ECDSA_SUPPORT == ENABLED || TLS_ECDH_ANON_SUPPORT)
...
/**
 * @brief Set operation mode (client or server) 选择客户端还是服务器模式
 * @param[in] context Pointer to the TLS context  TLS报文
 * @param[in] entity Specifies whether this entity is considered a client or a server 实体为客户端还是服务器
 * @return Error code
 **/
...
/**
 * @brief Initiate the TLS handshake 启动TLS握手
 * @param[in] context Pointer to the TLS context  TLS报文
 * @return Error code
 **/
...
/**
 * @brief Receive application data from a the remote host using TLS 使用TLS报文从远端主机接收应用程序数据
 * @param[in] context Pointer to the TLS context  TLS报文
 * @param[out] data Buffer into which received data will be placed  接收缓冲区
 * @param[in] size Maximum number of bytes that can be received  可收到的最大数据
 * @param[out] received Number of bytes that have been received   接收到的字节数
 * @param[in] flags Set of flags that influences the behavior of this function
 * @return Error code
 **/
