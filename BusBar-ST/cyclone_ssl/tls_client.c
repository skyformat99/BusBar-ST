...
error_t tlsSendClientHello(TlsContext *context)
{
   ...
   message->sessionIdLength = context->sessionIdLength;
   memcpy(message->sessionId, context->sessionId, context->sessionIdLength);
#else
   ...
   p += sizeof(TlsClientHello) + message->sessionIdLength;
   //Total length of the message
   length = sizeof(TlsClientHello) + message->sessionIdLength;
...
error_t tlsParseServerHello(TlsContext *context, const TlsServerHello *message, size_t length)
{
   ...
   if(message->sessionIdLength > 0 && message->sessionIdLength == context->sessionIdLength &&
      !memcmp(message->sessionId, context->sessionId, context->sessionIdLength))
   {
   ...
   context->sessionIdLength = message->sessionIdLength;
      ...
      context->keyExchMethod == TLS_KEY_EXCH_ECDHE_ECDSA)
   {
      ...
      if(context->version <= TLS_VERSION_1_1)
      {
            ...
            context->keyExchMethod == TLS_KEY_EXCH_ECDHE_RSA)
         {
            ...
            sha1Context = osAllocMem(sizeof(Sha1Context));
            //Failed to allocate memory?
         ...
         {
            //Allocate a memory buffer to hold the SHA-1 context
            Sha1Context *sha1Context = osAllocMem(sizeof(Sha1Context));
            //Failed to allocate memory?
            if(!sha1Context) return ERROR_OUT_OF_MEMORY;
      ...
      if(context->version == TLS_VERSION_1_2)
      {
         ...
         hashContext = osAllocMem(hashAlgo->contextSize);
         //Failed to allocate memory?
